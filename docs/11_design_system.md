...
| レイヤー順 | `status-radar.tsx` の `centerSlot` を **`ResponsiveContainer` より DOM 順で前**に配置。
兄弟要素は後に置いたものが上に描画されるため、SVG チャート（背景透明）が常に紋章の**上**に重なる |
...
**レイヤー順で頂点が隠れなくなる理由（半径比）**: `outerRadius="55%"`、375px 幅時のレーダー半径は約103px。
紋章半径は 48px/2=24px で、レーダー半径に対し約23%（Lv2.3相当）。旧仕様（64px・不透明・チャートより上のレイヤー）では
半径の約31%（Lv3.1相当）を不透明に覆い、Lv2〜3の頂点（BRIDGE・ENGLISH・PM・MARKETING）が隠れていた。
新仕様では (1) 紋章がチャートより下のレイヤーになったため、ポリゴン・グリッド・軸ラベルは常に紋章の**上**を通り、
