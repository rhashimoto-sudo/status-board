...
## 進行中
- [ ] 背景意匠の見た目確認 — team-lead によるブラウザ実測待ち（**ロットB に進まない**）
  - 状態: Wave 6 ロットA（#12〜#14）+ レビュー指摘10件の修正（#lotA-fix）+
    称号バグ/レーダー微調整/背景刷新（#titles-bg）まで合体済み（`f0b5a8e`）。
    `npm run verify` 通過 / Tests 139 passed / `check:cycles` 循環0件
  - 375px の横スクロールなし・軸ラベルのクリップ解消は team-lead が実測済み（AC-13 クリア）。
    `🔥 LEARNING` の左端2pxはみ出しは `outerRadius` 58% → 55% で解消
  - 次の一手: **背景意匠（system window 風）の見た目**を team-lead がブラウザで確認する。
    静的解析では確認できないのは (1) `body::before`/`::after` の `z-index: -1` が
    意図どおり描画されるか (2) テクスチャ上でテキストが WCAG AA を保つか の2点。
    仕様は `docs/11_design_system.md` に追記済み（Wave 9 の AC-14 監査で剥がされないため）
