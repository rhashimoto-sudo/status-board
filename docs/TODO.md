...
## 進行中
- [ ] パネル質感の見た目確認 — team-lead によるブラウザ実測待ち（**ロットB に進まない**）
  - 状態: Wave 6 ロットA（#12〜#14）+ レビュー指摘10件の修正（#lotA-fix）+
    称号バグ/レーダー微調整/背景刷新（#titles-bg）+ パネル質感3件（#panel-skin）まで
    合体済み（`90edfd0`）。`npm run verify` 通過 / Tests 139 passed / `check:cycles` 循環0件
  - team-lead が実測済み: 375px 横スクロールなし（AC-13）、軸ラベルのクリップ解消、
    総合称号が「実践者」で表示され称号バグの修正が実画面で有効
  - 次の一手: **背景意匠とパネル質感の見た目**を team-lead がブラウザで確認する。
    静的解析では確認できないのは (1) `body::before`/`::after` の `z-index: -1` が
    意図どおり描画されるか (2) 二重グラデと上辺シアン枠が「板の積層」に見えるか
    (3) 11px・`tracking-[0.18em]` の見出しが日本語＋絵文字で読みやすいか
    （`uppercase` は日本語・絵文字に効かない）の3点
