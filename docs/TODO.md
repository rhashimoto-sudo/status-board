...
- [ ] Wave 6 ロットC 着手待ち（ロットA・ロットB は完了。**ロットC に進む指示待ち**）
  - 状態: Wave 6 ロットA（#12〜#14）+ ロットB（#15〜#17）+ 各種指摘修正 +
    375px 横あふれの解消（#overflow375）まで合体済み（`2223241`）。
    `npm run verify` 通過 / Tests **153 passed / 7ファイル** / `check:cycles` 循環0件
  - **375px 実測は司令塔がブラウザで実施済み（AC-13 クリア）**:
    `width:375px; border:0` の iframe に一時確認ページを読み込ませて計測し、
    `scrollWidth` **391 → 375**（`clientWidth` 375 と一致・横スクロールなし）、
    右端を超える要素 **0件**（全 DOM 要素を走査）。ゲリラ行のタイトルは
    256px → 176px に縮み省略記号が機能。一時確認ページと `.next` は削除済みで
    `git status` は clean
  - team-lead が実測済み: TZ=UTC でも「月曜日」表示（本番バグ解消）、
    カウントダウンの毎秒動作、総合称号「実践者」、紋章 48px で9軸の頂点が視認可能、
  ...
  - **教訓（レイアウト検証）**: AC-13 のような CSS の受け入れ条件は `npm run verify` では
    一切検出できない。実際に描画して `scrollWidth <= clientWidth` を数値で確認するまで
    「通った」と言わないこと。flex の入れ子で `truncate` / `whitespace-nowrap` を使う場合は
    **親だけでなく当該要素自身にも `min-w-0`** が要る
