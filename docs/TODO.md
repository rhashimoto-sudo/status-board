...
- [ ] **S-1. レベルスケールの100段化（×10 リスケール方式）**
  ...
  **Lv100 の累積EXP = 11287 は据え置く**。これにより `DIFFICULTY_BASE_EXP`・HP増減・
  ...
  | `CALIBRATION_INITIAL_LEVEL_RANGE` | `{min:1, max:5}` → `{min:10, max:50}` |
  ...
  - 改訂が要る docs: `03_status_system.md` §2/§3.2 / `04_exp_rules.md` §4 /
...
- [ ] **S-2. ボスゲート（レベル上限解放）**
  ...
  **やり続ければ上がり続けるのを止める装置。** ボスを倒すまで上限に張り付く。
  ...
  - 実装: `level.ts` に cap 引数 / `GameState` に `levelCap` / `constants.ts` に
    `LEVEL_CAP_GATES = [30, 60, 90, 100]`
  - 画面: キャップ到達時は EXP バーを別扱いにし「⛔ Lv30 到達。ボス《…》討伐で解放」と
